...
class RagBot:
    ...
    def __init__(self):
        ...
        # 조문 번호 → 청크 id 목록 (예: "제39조" → ["chunk_87", "chunk_88"])
        # 임베딩은 번호를 잘 못 잡으므로 번호 질문은 이 색인으로 직접 찾는다
        all_meta = self.collection.get(include=["metadatas"])
        self.article_ids: dict[str, list[str]] = {}
        for cid, m in zip(all_meta["ids"], all_meta["metadatas"]):
            key = m["article"].split("(")[0]              # "제39조(보호기간의 원칙)" → "제39조"
            self.article_ids.setdefault(key, []).append(cid)
        for ids in self.article_ids.values():
            ids.sort(key=lambda x: int(x.split("_")[1]))  # ①②③ 순서 유지

    def lookup(self, query: str, where: dict | None = None) -> list[dict]:
        """질문에 조문 번호가 있으면 그 조문의 청크를 번호로 직접 가져옴"""
        found = []
        for num, sub in ART_NO.findall(query):
            key = f"제{num}조" + (f"의{sub}" if sub else "")
            ids = self.article_ids.get(key, [])
            if not ids:
                continue
            got = self.collection.get(ids=ids, where=where, include=["documents", "metadatas"])
            by_id = {i: (t, m) for i, t, m in zip(got["ids"], got["documents"], got["metadatas"])}
            for cid in ids:                                   # 원래 순서대로
                if cid in by_id:
                    t, m = by_id[cid]
                    found.append({"id": cid, "text": t, "meta": m, "similarity": None})
        return found
