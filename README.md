...
- `verify_citations`은 검색으로 **실제 반환된 청크**에서만 항(①②) 번호를 모은다. 한 조문이 여러 청크로 나뉘어 있는데 그중 일부만 검색 결과(top_k)에 뽑히면, 실제로는 맞는 인용인데도 "그 청크엔 없다"고 오탐(false positive)할 수 있다. 질문에 "제N조"가 직접 들어가면 `lookup()`이 그 조문의 청크를 전부 가져와서 이 위험이 줄어들지만, 순수 의미 검색만으로 일부만 뽑히는 경우엔 남아있다.
