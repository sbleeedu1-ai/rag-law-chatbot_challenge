...
`RagBot.verify_citations`는 "이 조문이 애초에 근거로 쓰였나"(환각 방지)는 검색 결과(`hits`)로 확인하지만, "그 조문에 항이 몇 개까지 있나"는 **top_k와 무관하게 DB에서 해당 (문서, 조문) 전체를 직접 조회**해서 확인한다. 한 조문이 여러 청크로 나뉘어 있고 그중 일부만 검색에 뽑혔더라도, 실제로 맞는 항 인용을 "그 청크엔 없다"고 오탐(false positive)하지 않는다. 대신 존재하지 않는 항을 인용하면(예: 항 기호가 아예 없는 조문에 "제2항"이라고 답하는 경우) 확실하게 걸러진다. 이 조회는 로컬 Chroma 메타데이터 조회라 LLM 토큰을 쓰지 않는다.
